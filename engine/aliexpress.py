...
class AliexpressScraping():
        ...
        for page in range(page_limit):
            ...
            try:
                for item in items_dict["mods"]["itemList"]["content"]:
                    items.append(
                        AliexpressItem(
                            url = self.ALIEXPRESS_DOMAIN_URL + item["productDetailUrl"][:item["productDetailUrl"].find("?")]
                            )
                        )
                #time.sleep(10)
            except Exception as e:
    ...
    def fetch_items(self, url: str, page_limit: int=10) -> typing.List[AliexpressItem]:
        ...
        for item in items:
            ...
            eel.view_log_js(f"{now_timestamp_jp}{count}/{len(items)}個目のアイテム情報を取得しました。") 
            count += 1
        
        eel.view_log_js(f"------------------------------------アイテム情報取得完了------------------------------------")
